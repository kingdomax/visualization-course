...
GLuint loadShaders(
    ...
    std::string const& fs,
    const int task_nbr,
    const int empty_space_skipping_toggle,
    const int adaptive_sampling_toggle,
    const int lighting_toggle
    )
{
    std::string v = readFile(vs);
    std::string f = readFile(fs);
    ...
    ss3 << empty_space_skipping_toggle;

    index = (int)f.find("#define ENABLE_EMPTY_SPACE_SKIPPING");
    f.replace(index + 36, 1, ss3.str());

    {
        std::stringstream ss4;
        ss4 << adaptive_sampling_toggle;

        index = (int)f.find("#define ENABLE_ADAPTIVE_SAMPLING");
        f.replace(index + 33, 1, ss4.str());
    }
...
float       g_iso_value_2 = 0.2f;

// set the light position and color for shading
glm::vec3   g_light_pos = glm::vec3(1.0, -10.0, -10.0);
glm::vec3   g_ambient_light_color = glm::vec3(0.3f, 0.3f, 0.3f);
glm::vec3   g_diffuse_light_color = glm::vec3(1.f, 1.f, 1.f);
...
// glm::ivec2  g_window_res = glm::ivec2(1200, 720);
glm::ivec2  g_window_res = glm::ivec2(2400, 1440);
...
bool g_empty_space_skipping_toggle = false;
bool g_adaptive_sampling_toggle = false;
...
const uint32_t min_max_accel_tex_unit = 2;
...
bool read_volume(std::string& volume_string){
    ...
    std::cout << "Volume Bounds: " << glm::to_string(g_max_volume_bounds) << std::endl;
...
volume_data_type create_min_max_acceleration_structure (glm::uvec3& accel_struct_dim, glm::vec3& accel_struct_cell_size){

    if (!g_volume_data.size()){
        std::cout << "ERROR: no volume data found" << std::endl;
        exit(-1);
    }
    if (!g_vol_dimensions.x || !g_vol_dimensions.y || !g_vol_dimensions.z ){
        std::cout << "ERROR: no volume dimensions found" << std::endl;
        exit(-1);
    }

    // set the size of the AS
    // const unsigned max_vol_dim = std::max(std::max(g_vol_dimensions.x, g_vol_dimensions.y), g_vol_dimensions.z);
    const glm::uvec3 voxels_per_accel_struct_cell_dim = glm::uvec3(32);
    accel_struct_dim = glm::uvec3(glm::ceil(glm::vec3(g_vol_dimensions) / glm::vec3(voxels_per_accel_struct_cell_dim) ));


    const uint32_t num_accel_struct_cells = accel_struct_dim.x * accel_struct_dim.y * accel_struct_dim.z;
    volume_data_type min_max_accel_struct (2 * num_accel_struct_cells);

    // find min and max values in each cell
    // make sure to check indices against volume dimensions
    for (uint32_t as_cell = 0; as_cell < num_accel_struct_cells; ++as_cell)
    {
        glm::ivec3 as_cell_3d_idx     = glm::ivec3(get_3d_index(as_cell, accel_struct_dim));

        // calculate start voxel, subtract 1 to account for neighbour cells
        glm::ivec3 start_voxel_3d_idx = as_cell_3d_idx * glm::ivec3(voxels_per_accel_struct_cell_dim) - glm::ivec3(1);

        uint8_t cell_min = 255;
        uint8_t cell_max = 0;

        for (uint32_t z = 0; z < voxels_per_accel_struct_cell_dim.z + 2; ++z){
            for (uint32_t y = 0; y < voxels_per_accel_struct_cell_dim.y + 2; ++y){
                for (uint32_t x = 0; x < voxels_per_accel_struct_cell_dim.x + 2; ++x){
                    
                    const glm::ivec3 voxel_3d_idx = start_voxel_3d_idx + glm::ivec3(x,y,z);

                    if ( glm::any(glm::greaterThanEqual(voxel_3d_idx, glm::ivec3(g_vol_dimensions))) ){
                        continue;
                    }
                    if (glm::any(glm::lessThan(voxel_3d_idx, glm::ivec3(0)))) {
                        continue;
                    }

                    const uint32_t voxel_1d_idx = get_1d_index(glm::uvec3(voxel_3d_idx), glm::uvec3(g_vol_dimensions));
                    const uint8_t voxel_val = g_volume_data[voxel_1d_idx];

                    cell_min = std::min(cell_min, voxel_val);
                    cell_max = std::max(cell_max, voxel_val);
                }
            }
        }

        //std::cout << "Min: " << cell_min << ", Max: " << cell_max << std::cout;

        // enter min and max into acceleration structure
        min_max_accel_struct[as_cell*2] = cell_min;
        min_max_accel_struct[as_cell*2+1] = cell_max;
    }


    glm::vec3 voxel_size = glm::vec3(g_max_volume_bounds)/glm::vec3(g_vol_dimensions);
    accel_struct_cell_size = voxel_size * glm::vec3(voxels_per_accel_struct_cell_dim); 

    return min_max_accel_struct;
}

GLuint upload_min_max_accel_struct_as_texture(const volume_data_type& min_max_accel_struct, const glm::uvec3 accel_struct_dim){

    glActiveTexture(GL_TEXTURE2);

    GLuint min_max_accel_tex;
    glGenTextures(1, &min_max_accel_tex);
    glBindTexture(GL_TEXTURE_3D, min_max_accel_tex);
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RG8, accel_struct_dim.x, accel_struct_dim.y, accel_struct_dim.z, 0, GL_RG, GL_UNSIGNED_BYTE, min_max_accel_struct.data());
    // glBindTexture(GL_TEXTURE_3D, 0);

    glActiveTexture(GL_TEXTURE0);

    return min_max_accel_tex;
}
...
void showGUI(){
    ...
    {        
        ...
        ImGui::RadioButton("Front-to-back Compositing", &g_task_chosen, 01);
        ImGui::RadioButton("Multi-isosurface Compositing", &g_task_chosen, 02);
        ImGui::RadioButton("First-hit Iso-Surface Ray-casting", &g_task_chosen, 03);
        ...
        if (ImGui::TreeNode("Settings")){    
            ...
            g_reload_shader ^= ImGui::Checkbox("Enable Empty Space Skipping", &g_empty_space_skipping_toggle); 
            g_reload_shader ^= ImGui::Checkbox("Enable Adaptive Sampling", &g_adaptive_sampling_toggle);
    ...
    {        
        if (ImGui::TreeNode("Window Size")){
            ...
            if (press){
                glm::ivec2 win_re_size = glm::ivec2(1920, 1080);
...
int main(int argc, char* argv[])
{
    ...
    glm::uvec3 accel_struct_dim;
    glm::vec3 accel_struct_cell_size;

    volume_data_type min_max_accel_struct = create_min_max_acceleration_structure (accel_struct_dim, accel_struct_cell_size);
    GLuint min_max_accel_tex = upload_min_max_accel_struct_as_texture(min_max_accel_struct, accel_struct_dim);

    const glm::vec3 accel_struct_bounds = glm::vec3(accel_struct_dim) * accel_struct_cell_size;

    std::cout << "Created acceleration struct with dimensions: " << glm::to_string(accel_struct_dim) << std::endl;
    std::cout << "Accel struct cell size: " << glm::to_string(accel_struct_cell_size) << std::endl;
    std::cout << "Accel struct bounds: " << glm::to_string(accel_struct_bounds) << std::endl;
    ...
    try {
        g_volume_program = loadShaders(g_file_vertex_shader, g_file_fragment_shader,
            g_task_chosen,
            g_empty_space_skipping_toggle,
            g_adaptive_sampling_toggle,
            g_lighting_toggle
    ...
    while (!g_win.shouldClose()) {
        ...
        if (g_reload_shader){
            ...
            try {
                ...
                newProgram = loadShaders(g_file_vertex_shader, 
                                         g_file_fragment_shader, 
                                         g_task_chosen, 
                                         g_empty_space_skipping_toggle, 
                                         g_adaptive_sampling_toggle,
                                         g_lighting_toggle);
        ...
        glUniform1i(glGetUniformLocation(g_volume_program, "min_max_accel_texture"), 2);
        ...
        glUniform3uiv(glGetUniformLocation(g_volume_program, "accel_struct_dims"), 1,
                    glm::value_ptr(accel_struct_dim));
        glUniform3fv(glGetUniformLocation(g_volume_program, "accel_struct_cell_size"), 1,
                    glm::value_ptr(accel_struct_cell_size));
        glUniform3fv(glGetUniformLocation(g_volume_program, "accel_struct_bounds"), 1,
                    glm::value_ptr(accel_struct_bounds));        
    ...
    glDeleteTextures(1,&min_max_accel_tex);
